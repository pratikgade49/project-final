  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const algorithmMap: Record<string, string> = {
        ...
        "Best Fit (Auto-Select)": "best_fit",
        "Ensemble (Top 3 Avg)": "best_fit"
      };
      const backendAlgorithm =
        result.selectedAlgorithm.includes("Best Fit") || result.selectedAlgorithm.includes("Ensemble")
          ? "best_fit"
          : algorithmMap[result.selectedAlgorithm as string] || "linear_regression";
      const config = {
        forecastBy,
        selectedItem,
        algorithm: backendAlgorithm,
        interval: result.historicData[0]?.period?.toLowerCase().includes('week') ? 'week' : result.historicData[0]?.period?.toLowerCase().includes('year') ? 'year' : 'month',
        historicPeriod: result.historicData.length,
        forecastPeriod: result.forecastData.length
      };
