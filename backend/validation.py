...
class DateRangeValidator:
    ...
    @staticmethod
    def validate_factor_coverage(db: Session, factor_names: List[str]) -> Dict[str, Dict]:
        """Validate that external factors have adequate coverage of main data date range"""
        main_min, main_max = DateRangeValidator.get_main_data_date_range(db)
        
        if not main_min or not main_max:
            return {"error": "No main forecast data found"}
